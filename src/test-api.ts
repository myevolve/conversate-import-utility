import { ConversateAPI } from './lib/api';

async function testAPI() {
  const api = new ConversateAPI();
  ...
  try {
    ...
    console.log('Accounts:', loginResponse.data.accounts);
    const account = loginResponse.data.accounts.find(a => a.name === 'Conversate Sales Demo Account');
    if (!account) {
